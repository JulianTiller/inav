...
#ifdef AURIX
void onSerialRxPinChange(timerCCHandlerRec_t *cbRec, captureCompare_t capture_rise, captureCompare_t capture_fall);
#else
...
{
#ifdef AURIX
	softSerial->timerHardware->tim->tim.CTRL.B.TIM_EN = 1;
#else
#ifdef USE_HAL_DRIVER
...
#ifdef AURIX
    if (softSerial->port.options & SERIAL_INVERTED) {
        const uint8_t pinConfig = (softSerial->port.options & SERIAL_BIDIR_NOPULL) ? IOCFG_IN_FLOATING : IOCFG_IPD;
        IOConfigGPIO(softSerial->rxIO, pinConfig);
    } else {
        const uint8_t pinConfig = (softSerial->port.options & SERIAL_BIDIR_NOPULL) ? IOCFG_IN_FLOATING : IOCFG_IPU;
        IOConfigGPIO(softSerial->rxIO, pinConfig);
    }
#else
    if (softSerial->port.options & SERIAL_INVERTED) {
...
#ifdef AURIX
	softSerial->timerHardware->tim->tim.CTRL.B.TIM_EN = 0;
...
#ifdef AURIX
    if (softSerial->exTimerHardware && softSerial->exTimerHardware->tim != softSerial->timerHardware->tim) {
#else
...
void onSerialRxPinChange(timerCCHandlerRec_t *cbRec, captureCompare_t capture)
{
    ...
    if (self->isSearchingForStartBit) {
        ...
        // of the bit period.
#ifdef AURIX
    	if (self->exTimerHardware)
    		self->exTimerHardware->tim->tom.CN0.U = self->exTimerHardware->tim->tom.CM0.U >> 1;
#else
#ifdef USE_HAL_DRIVER
