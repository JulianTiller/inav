...
#define OSD_LABEL_ENTRY(label)                  { label, {.func = NULL}, NULL, OME_Label, 0 }
#define OSD_LABEL_DATA_ENTRY(label, data)       { label, {.func = NULL}, data, OME_Label, 0 }
#define OSD_LABEL_DATA_DYN_ENTRY(label, data)   { label, {.func = NULL}, data, OME_Label, DYNAMIC }
#define OSD_LABEL_FUNC_DYN_ENTRY(label, fn)     { label, {.func = NULL}, fn, OME_LabelFunc, DYNAMIC }
#define OSD_BACK_ENTRY                          { "BACK", {.func = NULL}, NULL, OME_Back, 0 }
#define OSD_BACK_AND_END_ENTRY                  { "BACK", {.func = NULL}, NULL, OME_BACK_AND_END, 0 }
#define OSD_SUBMENU_ENTRY(label, menu)          { label, {.func = NULL}, menu, OME_Submenu, 0 }
#define OSD_FUNC_CALL_ENTRY(label, fn)          { label, {.func = fn}, NULL, OME_Funcall, 0 }
#define OSD_BOOL_ENTRY(label, val)              { label, {.func = NULL}, val, OME_Bool, 0 }
#define OSD_BOOL_CALLBACK_ENTRY(label, cb, val) { label, {.func = cb}, val, OME_Bool, 0 }
#define OSD_BOOL_FUNC_ENTRY(label, fn)          { label, {.func = NULL}, fn, OME_BoolFunc, 0 }
#define OSD_INT8_DYN_ENTRY(label, val)          { label, {.func = NULL}, val, OME_INT8, DYNAMIC }
#define OSD_UINT8_ENTRY(label, val)             { label, {.func = NULL}, val, OME_UINT8, 0 }
#define OSD_UINT8_DYN_ENTRY(label, val)         { label, {.func = NULL}, val, OME_UINT8, DYNAMIC }
#define OSD_UINT8_CALLBACK_ENTRY(label, cb, val){ label, {.func = cb}, val, OME_UINT8, 0 }
#define OSD_UINT16_ENTRY(label, val)            { label, {.func = NULL}, val, OME_UINT16, 0 }
#define OSD_UINT16_DYN_ENTRY(label, val)        { label, {.func = NULL}, val, OME_UINT16, DYNAMIC }
#define OSD_UINT16_RO_ENTRY(label, val)         { label, {.func = NULL}, val, OME_UINT16, DYNAMIC | READONLY }
#define OSD_INT16_ENTRY(label, val)             { label, {.func = NULL}, val, OME_INT16, 0 }
#define OSD_INT16_DYN_ENTRY(label, val)         { label, {.func = NULL}, val, OME_INT16, DYNAMIC }
#define OSD_INT16_RO_ENTRY(label, val)          { label, {.func = NULL}, val, OME_INT16, DYNAMIC | READONLY }
#define OSD_STRING_ENTRY(label, str)            { label, {.func = NULL}, str, OME_String, 0 }
#define OSD_TAB_ENTRY(label, val)               { label, {.func = NULL}, val, OME_TAB, 0 }
#define OSD_TAB_DYN_ENTRY(label, val)           { label, {.func = NULL}, val, OME_TAB, DYNAMIC }
#define OSD_TAB_CALLBACK_ENTRY(label, cb, val)  { label, {.func = cb}, val, OME_TAB, 0 }

#define OSD_END_ENTRY                           { NULL, {.func = NULL}, NULL, OME_END, 0 }
